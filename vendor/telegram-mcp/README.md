...
- **Custom emoji:** search by keyword or base emoji, browse categories and packs, inspect IDs in messages and polls, render or download previews, choose a random emoji from Telegram-validated results, send emoji and reactions, manage emoji status, and install/archive packs. Long Telegram IDs are also returned as decimal strings for lossless MCP/JSON reuse; arbitrary random numbers are never treated as emoji IDs.
