...
Временный Gemini `429` повторяется с задержками 10 и 20 секунд внутри общего
45-секундного лимита. Исчерпанная квота не повторяется вслепую, а карточка
показывает честную причину вместо сообщения об ошибке разбора команды.
